...
def _apply_ocr_in_images(imagens):
    texto_extraido: list[str] = []
...
def _apply_ocr_to_image(imagem):
...
class AzOptipy:
    ...
    def precess_ocr(self):
        if self._file_type == FileType.PDF:
            return _process_pdf(self._path)
        elif self._file_type == FileType.JPEG or self._file_type == FileType.PNG:
            return _process_img(self._path)
        else:
            raise ValueError("extensão nao Identificada.")

    def precess_base64_ocr(self):
        if self._file_type == FileType.TXT:
            extension, decoded_data = _process_base64(self._path)

            if extension == FileType.PDF.value:
                pdf_base64 = Base64ToPDF.get_pdf_to_images(decoded_data)
                imagens = PDF2Images.get_pdf_base64_to_images(pdf_base64)

                return _apply_ocr_in_images(imagens)

            elif extension == FileType.JPEG.value or extension == FileType.PNG.value:
                image_file = io.BytesIO(decoded_data)
                imagem = Image.open(image_file)
                return _apply_ocr_to_image(imagem)
            else:
                raise ValueError("extensão nao Identificada.")
        else:
            raise ValueError("extensão nao Identificada.")
